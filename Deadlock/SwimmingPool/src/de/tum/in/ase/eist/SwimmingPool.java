...
public class SwimmingPool {
    ...
    private static final SwimmingPoolActionOrder PRESCRIBED_ORDER = SwimmingPoolActionOrder.CHANGING_ROOM_BEFORE_LOCKER;
    ...
    public void handleEntryRequestDeadlockFree(Swimmer swimmer, SwimmingPoolActionOrder order) {
        // TODO 3
        if (order == PRESCRIBED_ORDER) {
            handleEntryRequest(swimmer, order);
        } else {
            System.out.println("Swimmer " + swimmer.getId() + " has been rejected because of the wrong order.");
            System.out.println("Prescribed order: " + PRESCRIBED_ORDER + ", swimmer order: " + order + ".");
        }
    }
